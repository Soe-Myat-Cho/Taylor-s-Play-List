...
const Controls = () => {
  ...
  return (
    <>
      {/* Progree Bar Section */}
      <div className="relative h-1 bg-gray-400 rounded mb-4">
        <div
          className="absolute top-0 left-0 h-full bg-blue-400 shadow-sm shadow-blue-400 rounded"
          style={{ width: `${(currentTime / duration) * 100}%` }} // Update width based on current time
        >
          <span className=" absolute right-0 top-[-100%] w-3 h-3 bg-blue-300 rounded-full"></span>
        </div>
      ...
      <div className="flex justify-around items-center mt-10">
        ...
        <button onClick={prevSong} className=" ">
          <FaStepBackward className=" hover:text-blue-600 duration-300" />
        </button>
        ...
        <button onClick={nextSong} className=" ">
          <FaStepForward className=" hover:text-blue-600 duration-300" />
        </button>
      </div>
    </>
  );
